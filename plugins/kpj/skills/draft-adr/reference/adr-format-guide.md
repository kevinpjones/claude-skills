...
- **Formal third person. No "I" or "the user."** An ADR is a team artifact that outlives the conversation that produced it. Write "the trigger fires on every UPDATE" or "we chose X because Y," never "I found that..." or "the user wants...". Keep the register formal and objective — plain and direct, but not conversational or casual. If you're revising a draft based on feedback, the fix belongs in the sentence itself — not as a note explaining that a correction was made.
- **No process narration.** Don't leave commentary aimed at whoever is reviewing the draft in conversation — no "removed per feedback," no "this was internal commentary only," no explaining why a sentence changed. If a sentence was there only to justify an edit to the person reviewing it, delete it rather than leaving it in the merged file; it reads as a leftover the moment someone encounters it without that context.
- **Verify causal claims before writing them down.** Don't fill a gap in the Context or Tradeoffs with a plausible-sounding guess about why something happens or what a rejected alternative would or wouldn't have fixed. If you haven't confirmed the actual mechanism — via code exploration, existing docs, or the person who requested the ADR — say so explicitly or ask, rather than asserting it as settled fact. A wrong root cause, or an alternative rejected for a reason that turns out not to be true (e.g., "prevents duplicate billing" when idempotency already handles that), undermines the whole record for anyone who knows the system.