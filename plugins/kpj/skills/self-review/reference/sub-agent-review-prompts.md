...
General findings are collected and presented in the review summary rather than as inline issues.
...
```
Review this diff for opportunities to simplify and improve code quality:
...
2. **Duplication**: Find repeated patterns within the diff itself that could be extracted into shared functions.
...
4. **Dead code**: Identify unused imports, unreachable branches, or unnecessary assignments introduced by these changes.
...
```
Review this diff for correctness and robustness issues:
...
```
Review this diff for TypeScript type safety and best practices:
...
```
Review this diff for efficiency and performance issues:
...
4. **Algorithm complexity**: Identify potential O(n^2) or worse patterns that could be optimized.
