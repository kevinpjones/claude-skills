...
- `model` and `allowed-tools` are optional (can specify if needed; `tools` is the legacy sub-agent equivalent)
...
#### Remove/Transform
- **Keep or remove**: `model:` field (valid for skills if you want to specify a model)
- **Convert**: `tools:` field → `allowed-tools:` if needed (the `tools` field is legacy sub-agent format)
